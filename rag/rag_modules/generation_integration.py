...
class GenerationIntegrationModule:
    ...
    def __init__(self, model_name: str = "intern-s1", temperature: float = 0.1, max_tokens: int = 2048):
        """
        初始化生成集成模块
    ...
    def setup_llm(self):
        ...
        if not api_key:
            raise ValueError("请设置 INTERN_API_KEY 环境变量")
...
回答:""")
        ...
        # 使用LCEL构建链
        chain = (
            {"question": RunnablePassthrough(), "context": lambda _: context}  #这里lambda作用是函数直接返回context，相当于每次都重新计算context
            | prompt
            | self.llm
            | StrOutputParser()
        )
